(function ($) {
	var weekdays = ['Mond', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
			...
			this.bookedDates.forEach(function (date) {
				if (date.start <= lastDate && date.end >= firstDate) {
					...
					if (startIndex !== undefined) {
						self.$cells.eq(startIndex).addClass('unavailable');
						++startIndex;
					}
					else {
					...
					if (endIndex !== undefined) {
						self.$cells.eq(endIndex).addClass('unavailable');
						--endIndex;
					}
					else {
					...
					self.$cells.slice(startIndex, endIndex + 1).addClass('unavailable');
				}
			});
		},
